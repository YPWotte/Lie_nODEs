...
# <h1> Only General Potential Shaping: gradients immediately on algebra, potential on SE(3), full-state integral loss</h1>
...
### Initialize Dynamics 
(I,B,V) = torch.load('IBV_fShaping_21_07_10:07.pt') #('IBV_fShaping_19_09_16:25.pt')
...
aug_f = AugmentedSE3(f, IntegralLoss_Full(f,i_su,i_sw,i_sd,i_sPw,i_sPv,H_target),target).to(device) 
...
model = NeuralODE_Hybrid(f, jspan, callbacks, jspan_adjoint, callbacks_adjoint, solver, atol, rtol, dt_min, atol_adjoint, rtol_adjoint, dt_min_adjoint, IntegralLoss_Full(f,i_su,i_sw,i_sd,i_sPw,i_sPv,H_target), sensitivity = 'hybrid_adjoint_full').to(device) 
...
#trainer = pl.Trainer(max_epochs=1000, logger=logger, gpus = torch.cuda.device_count())#
#trainer.fit(learn)
